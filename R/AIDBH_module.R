...
AIDBH_module <- function(BALc=NA, SDI=NA, DBH=NA, A=NA, PS=NA, DA=NA, PSCAL=NA,SP=NA, ZONE=NA, Model=NA){
  ...
  #           + b8*SPZONE32+ b9*SPZONE33+ b10*SPZONE34 + b11*(BALc+10) + b12*SDI + b13*log(DBH) + b14*log(A) + b15*PS
  ...
  # Model 4 (LASSO with SPZONE): Log(AIDBH) = b0 + b1*PS + b2*(BALc+10) + b3*SDI + b4*log(DA) + b5*log(DBH) + b6*log(A) + b7*(1/sqrt(PSCAL+10)
