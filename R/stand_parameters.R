...
#' @param area: Area of plot in square meters
...
stand_parameters <- function(stand, area,
                             species.col = 'ESPECIE', dbh.col = 'DAP', height.col = 'HT'){
  ...
  HD <- mean(tail(stand[order(stand[height.col], decreasing = TRUE), height.col], CF))
  ...
  return(data.frame(BA, NHA, DQ, HD, Noth_prop, Dominant))
}
